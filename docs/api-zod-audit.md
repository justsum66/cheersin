...
以下路由使用 `request.json()` 後以 TypeScript 型別斷言或手動檢查，建議逐步補上 Zod schema.parse 或 safeParse。**優先級**：P1 = 支付/訂閱/遊戲房間核心；P2 = learn、recommend；P3 = 其餘。

| 路徑 | 現狀 | 優先級 |
|------|------|--------|
| /api/games/rooms/[slug]/script-murder | body 手動檢查 action | P1 |
| /api/recommend | body as RecommendPostBody | P2 |
| /api/report | body as ReportPostBody | P2 |
| /api/learn/notes | body 手動 | P2 |
| /api/learn/certificate | body 手動 | P2 |
| /api/subscription/promo | body as SubscriptionPromoPostBody | P2 |
...
- [x] 其餘路由依優先級（P1 > P2 > P3）逐個補 schema.parse 或 safeParse，並回傳 400 + 統一錯誤格式；進行中見上方「尚未使用 Zod」表格。
