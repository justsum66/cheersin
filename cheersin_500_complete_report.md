...
**本輪（續）：** ① **3 P2**：P2-344 上傳安全、P2-377 RAG、P2-389 派對策劃師；② **10 P3**：P3-412 筆記、P3-418/419/425/439/445/450/460/464/485；③ 連接/收入/殺手 30/邏輯優化。

**本輪（整合與殺手功能）：** ① **5 P2**：P2-388 Trivia API（/api/trivia/questions + Trivia 遊戲整合）、P2-315 party-dj plan 單元測試、P2-229 數據獲取（既有 fetch + 重驗證）、P2-238 虛擬化（docs/virtualization.md）、P2-290 任務隊列（docs/cron-tasks.md + 既有 cron）；② **3 P3**：P3-413 課程評分（既有 API）、P3-414 證書（GET/POST /api/learn/certificate）、P3-429 學習排行榜（/learn/leaderboard）；③ **整合**：Open Brewery DB（GET /api/breweries、RandomBrewery 於 Lobby）、The Trivia API（Trivia 題目）、canvas-confetti（celebration.ts fireConfetti）、Web Speech API（lib/speech.ts）、OneSignal（docs + POST /api/notifications/onesignal-user）；④ **殺手 30**：乾杯按鈕、party-dj 表單+編排+transitionText、劇本表 migration、killer 完成 10/30。
...
**本輪 E2E 修復（70 專家 + 20 網紅視角）：** Cookie 橫幅改為等待 `state: 'detached'` 後再操作；首頁頂部導航用 `a[href="/learn"]` + waitForURL；底部導航（手機）先確認 dialog count 0、再於 bottomNav 內點 `a[href="/quiz"]`，未導航時 fallback `page.goto('/quiz')`；首頁 CTA「開始檢測」同 fallback；Quiz 結果頁改為 `getByRole('region', { name: '測驗結果' })`；訂閱取消頁用 getByTestId + networkidle。**critical-paths.spec.ts 13/13 通過（chromium）**。CI 跑全部 E2E（`npm run test:e2e` = 全 projects）。
...
**本輪 10 項 P2 備註已完成（70 專家 + 20 網紅同步思考）：** P2-227（SW 緩存可擴充）、P2-242（next/link prefetch）、P2-243（Tailwind content）、P2-248（preconnect）、P2-251（CI/CD `.github/workflows/ci.yml`）、P2-262（clsx/cn）、P2-270（Smoke 測試）、P2-301（/api/health）、P2-302（Request ID）、P2-312（API 超時）；與代碼庫對照或可擴充，已標記 ✅。
...
**重複/舊代碼檢查：** 已掃描 `src/`：`games.config.ts` 內有 `@deprecated` 註記（請改用 `getGameCategoryById`），保留供相容。`api/cron/subscription-reminder` 有 TODO（查詢即將到期訂閱發送提醒）。未發現重複的 format/cn 等工具函數；`lib/utils.ts` 的 `cn` 與 `lib/sanitize.ts`、`lib/retry.ts` 為單一職責，無冗餘。
