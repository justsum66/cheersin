...
interface WineCardProps {
  ...
  /** Intersection Observer 用於延遲加載 */
  inView?: boolean;
}

/** 
 * 任務 8：優化 WineCard 圖片加載策略
 * 實現：
 * 1. 漸進式圖片加載（blur placeholder）
 * 2. Intersection Observer 延遲加載
 * 3. 響應式圖片尺寸優化
 * 4. 錯誤處理和降級顯示
 */
const OptimizedWineCardInner = function OptimizedWineCard({
  wine,
  onAddToWishlist,
  inWishlist = false,
  similar = [],
    ...
    <div className={variant === 'horizontal' ? 'w-28 sm:w-36 shrink-0 aspect-square relative bg-white/5 rounded-t-xl overflow-hidden' : 'aspect-[4/3] relative bg-white/5 rounded-t-xl overflow-hidden'}>
      {wine.imageUrl && !imageError ? (
        <Image
          ...
          placeholder="blur"
          blurDataURL="data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMzIwIiBoZWlnaHQ9IjI0MCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iMTAwJSIgaGVpZ2h0PSIxMDAlIiBmaWxsPSIjMzMzIi8+PC9zdmc+"
        />
      ) : (
      ...
      {/* 漸進式加載指示器 */}
      {!imageLoaded && wine.imageUrl && !imageError && (
...
export const OptimizedWineCard = memo(OptimizedWineCardInner);