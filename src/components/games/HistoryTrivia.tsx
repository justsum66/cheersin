...
import { shuffleHistoryTrivia, type HistoryTriviaItem } from '@/data/history-trivia'

const QUESTION_COUNT = 8

/** R2-146：歷史知識問答喝酒版 — 專用歷史冷知識題庫，答錯喝 */
export default function HistoryTrivia() {
  const { play } = useGameSound()
  const reducedMotion = useGameReduceMotion()
  const punishment = usePunishmentCopy()
  const [questions, setQuestions] = useState<HistoryTriviaItem[]>(() => shuffleHistoryTrivia(QUESTION_COUNT))
  ...
  const restart = useCallback(() => {
    if (nextTimeoutRef.current) clearTimeout(nextTimeoutRef.current)
    play('click')
    setQuestions(shuffleHistoryTrivia(QUESTION_COUNT))
    setCurrent(0)
    setSelected(null)
    setIsAnswered(false)
    setScore(0)
  ...
  return (
    <div className="flex flex-col items-center h-full py-4 md:py-6 px-4 safe-area-px" role="main" aria-label="歷史知識問答">
      <GameRules
        rules="歷史冷知識選擇題，答對得分、答錯喝一口，再下一題。"
        rulesKey="history-trivia.rules"
      />
      <BookOpen className="w-12 h-12 text-primary-400 mb-2" />
      <p className="text-white/60 text-sm mb-2">歷史知識問答 · 答錯喝</p>
        ...
        <m.div key={current} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-lg">
          ...
          {isWrong && (
            <div className="mt-4 text-center">
              <p className="text-red-300 font-medium">答錯了～{punishment.drinkOne}！</p>
