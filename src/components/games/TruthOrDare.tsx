...
export default function TruthOrDare() {
  ...
  // Load Initial Data
  useEffect(() => {
    const tPool = getTruthPool()
    const dPool = getDarePool()
    setTruthPool(tPool)
    setDarePool(dPool)
  ...
  }, [mode, pick, punishment, setStats, players, skipCount, play, isSkipPending])
                ...
                <div className="mt-3 p-3 rounded-xl bg-white/5 border border-white/10 space-y-2" role="region" aria-label="自訂題目">
                  <div className="flex flex-wrap gap-2">
                    <input
                      ...
                      aria-label="真心話題目"
                    />
                    <motion.button whileHover={buttonHover} whileTap={buttonTap} type="button" onClick={() => addCustom('truth')} className="min-h-[48px] min-w-[48px] px-3 rounded-lg bg-primary-500 text-white text-sm font-medium games-focus-ring" aria-label="新增真心話">新增</motion.button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <input
                      type="text"
                      ...
                      aria-label="大冒險題目"
                    />
                    <motion.button whileHover={buttonHover} whileTap={buttonTap} type="button" onClick={() => addCustom('dare')} className="min-h-[48px] min-w-[48px] px-3 rounded-lg bg-primary-500 text-white text-sm font-medium games-focus-ring" aria-label="新增大冒險">新增</motion.button>
                  </div>
          ...
          >
            ...
            <div className={`p-4 md:p-6 lg:p-10 rounded-2xl md:rounded-[2rem] border backdrop-blur-3xl shadow-2xl relative overflow-hidden cursor-grab active:cursor-grabbing safe-area-px ${mode === 'truth' ? 'bg-pink-900/20 border-pink-500/30' : 'bg-orange-900/20 border-orange-500/30'
              ...
              <div className="flex gap-3 sm:gap-4 w-full max-w-lg flex-wrap sm:flex-nowrap">
                ...
                <motion.button whileHover={buttonHover} whileTap={buttonTap} type="button" onClick={backToMenu} className="flex-1 min-h-[48px] min-w-[48px] btn-primary inline-flex items-center justify-center gap-2 games-focus-ring" data-testid="truth-or-dare-next-round">
