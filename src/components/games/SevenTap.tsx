...
export default function SevenTap() {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full py-4 md:py-6 px-4 safe-area-px" role="main" aria-label="七拍">
      ...
      {phase === 'playing' && (
        <m.button type="button" onClick={handleTap} className="min-h-[64px] min-w-[64px] rounded-full bg-primary-500 text-white text-2xl font-bold games-focus-ring" data-testid="seven-tap-btn" aria-label={`點擊，目前 ${taps} 下`} whileTap={reducedMotion ? undefined : { scale: 0.95 }}>
          {taps} / {TARGET_TAPS}
        </m.button>
      )}
      {phase === 'result' && result && (
