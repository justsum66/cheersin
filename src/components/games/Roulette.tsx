...
function StandardRoulette() {
  ...
  // No legacy wrappers
  // trial is now the store object { roundsLeft, isGuestTrial }
  ...
  const spin = useCallback(() => {
    ...
    spinTimeoutRef.current = setTimeout(() => {
      ...
      setIsSpinning(false)
      setStats({ durationSec: reducedMotion ? 1 : sec })
  ...
  }, [isSpinning, players, spinDuration, reducedMotion, playSound, setGameState, setStats, addReplayEvent, trial, decrementTrialRound])
  ...
  const handlePointerUp = useCallback(
    ...
    [isSpinning, players, reducedMotion, applyWheelTransform, playSound, addReplayEvent]
  )
          ...
          <div className="flex flex-wrap gap-2">
            {historySlice.map((name, i) => (
              <m.button
                ...
                className="min-h-[48px] min-w-[48px] px-2 py-1 rounded-lg bg-white/10 text-white/80 text-sm hover:bg-white/20 transition-colors"
                aria-label={`複製 ${name}`}
              >
                {name}
              </m.button>
      ...
      {!isSpectator && (
        <m.div variants={slideUp} className="flex flex-wrap gap-2 justify-center">
          <m.button
            ...
            whileHover={!isSpinning && players.length >= 2 ? buttonHover : undefined}
            whileTap={!isSpinning && players.length >= 2 ? buttonTap : undefined}
            className="btn-primary w-56 md:w-64 min-h-[48px] text-lg md:text-xl shadow-glow-primary disabled:opacity-50 disabled:cursor-not-allowed games-focus-ring"
          ...
          <m.button
            ...
            whileHover={!isSpinning && players.length >= 2 ? buttonHover : undefined}
            whileTap={!isSpinning && players.length >= 2 ? buttonTap : undefined}
            className="min-h-[48px] px-4 py-2 rounded-xl bg-white/10 border border-white/20 hover:bg-white/20 text-white text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed games-focus-ring"
            aria-label="再轉一次"
            title="或按 R 鍵、搖一搖手機"
          >
            再轉一次 (R)
        ...
        <m.form variants={slideUp} onSubmit={addPlayer} className="mt-6 flex flex-wrap gap-2 w-full max-w-xs" aria-label="新增玩家">
          <div className="flex gap-2 flex-1 min-w-0 flex-wrap sm:flex-nowrap">
            ...
            <m.button
              ...
              className="shrink-0 min-h-[48px] min-w-[48px] px-4 py-2 bg-white/10 hover:bg-white/20 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-white font-bold transition-colors games-focus-ring"
              aria-label="新增玩家"
            >
              +
            </m.button>
          ...
          <m.button
            ...
            type="button"
            onClick={() => setPlayers([])}
            className="min-h-[48px] min-w-[48px] px-4 py-2 bg-red-500/20 hover:bg-red-500/40 text-red-500 rounded-lg transition-colors text-sm games-focus-ring"
            aria-label="清空玩家名單"
          >
            清空名單
          </m.button>
