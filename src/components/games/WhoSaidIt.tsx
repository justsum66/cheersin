...
import { pickRandomWhoSaidIt, type WhoSaidItItem } from '@/data/who-said-it'

const ROUND_SIZE = 6

/** R2-158：誰說的 — 顯示語錄→多選一猜誰說的→答錯喝 */
export default function WhoSaidIt() {
  const { play } = useGameSound()
  const reducedMotion = useGameReduceMotion()
  const [items, setItems] = useState<WhoSaidItItem[]>(() => pickRandomWhoSaidIt(ROUND_SIZE))
  const [index, setIndex] = useState(0)
  const [picked, setPicked] = useState<number | null>(null)
  const [revealed, setRevealed] = useState(false)

  const item = items[index]
  const correct = item && picked !== null && item.options[picked] === item.person
  ...
  const handlePick = useCallback(
    (optIndex: number) => {
      ...
      setPicked(optIndex)
      setRevealed(true)
      const right = item.options[optIndex] === item.person
  ...
  const restart = useCallback(() => {
    play('click')
    setItems(pickRandomWhoSaidIt(ROUND_SIZE))
    setIndex(0)
    setPicked(null)
    setRevealed(false)
  }, [play])
  ...
  return (
    <div className="flex flex-col items-center h-full py-4 md:py-6 px-4 safe-area-px" role="main" aria-label="誰說的">
      <GameRules
        rules="看語錄從選項中猜是誰說的，猜錯揭曉後喝一口。"
        rulesKey="who-said-it.rules"
      />
      <Quote className="w-12 h-12 text-primary-400 mb-2" />
      <p className="text-white/60 text-sm mb-2">誰說的</p>
          ...
          {!revealed ? (
            ...
            <AnimatePresence>
              <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
                <p className="text-white/80 mb-1">答案：{item.person}</p>
