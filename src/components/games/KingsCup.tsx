...
export default function KingsCup() {
  ...
  /** 36–39 Focus trap：第四張 K / 重置確認 Modal 焦點不逸出 */
  useEffect(() => {
    if (!showFourthKing && !showResetConfirm) return
    const container = showFourthKing ? fourthKingModalRef.current : resetConfirmModalRef.current
    if (!container) return
  ...
  }, [showFourthKing, showResetConfirm])
  ...
  const drawCard = () => {
    ...
    setDrawHistory((prev) => [
      ...prev,
      { card: card.card, name: card.name, short: card.short, emoji: card.emoji, isFourthKing }
    ])
    if (card.card === 'K') {
  ...
  const reset = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full py-4 md:py-6 px-4 safe-area-px" role="main" aria-label="國王遊戲">
            ...
            <div className="relative mb-8 md:mb-12 w-full max-w-[16rem] sm:w-64 h-72 sm:h-80 mx-auto">
                {/* Deck Stack */}
                ...
                {/* 51 3D 撲克牌翻牌：牌背 -> 牌面；#62 結果區 testid；#67 減少動畫 */}
                <AnimatePresence>
                    {currentCard && (
                        <motion.div
                        ...
                        >
                            {/* RWD-52：牌面固定比例，避免小螢幕變形 */}
                            <motion.div
                                className="relative w-40 md:w-48 aspect-[5/7] rounded-2xl shadow-2xl"
                                style={{ transformStyle: 'preserve-3d' }}
                                initial={reducedMotion ? false : { rotateY: 180 }}
                                animate={{ rotateY: 0 }}
                                transition={reducedMotion ? { duration: 0 } : { duration: 0.5, ease: 'easeOut' }}
                            >
                                {/* G3D-KingsCup-03：牌面 K/Q/J 數字排版精緻化 */}
            ...
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6 md:gap-8 mb-6 text-center">
                ...
                <div className="flex flex-col items-center">
                    {/* G3D-KingsCup-05：公杯視覺 3D 或漸層、進度條精綻化 */}
                    {/* Phase 1 C4.2: 進度條動畫 - 公杯液體波動 */}
            ...
            <div className="flex flex-wrap gap-3 md:gap-4 justify-center w-full max-w-md">
                ...
                {/* G3D-KingsCup-07：抽牌/重置按鈕 3D 風格、hover */}
                ...
                {showResetConfirm && (
                    ...
                    >
                        <motion.div
                            ...
                            tabIndex={-1}
                        >
                            {/* G3D-KingsCup-12：重置確認 Modal 排版與 CTA 階層 */}
