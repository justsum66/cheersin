...
interface CourseListVirtualizedProps {
...
const CourseCard = ({ 
  ...
  const freeTierOverLimit = tier === 'free' && !course.free // Assuming free tier has limited access to pro courses
  ...
  return (
    <m.div
      ...
      onClick={() => !isLocked && onCoursePreview(course)}
    ...
    >
      ...
      >
        ...
        {/* Difficulty tag */}
        <span className="absolute bottom-3 left-3 text-[10px] px-2 py-0.5 rounded-full bg-white/15 text-white/90 z-20">{DEFAULT_LEVEL_LABELS[course.level]}</span>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/60 font-medium">
                {DEFAULT_LEVEL_LABELS[course.level]}
              </span>
              {course.level === 'expert' && (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-violet-500/20 text-violet-300" title="深度內容">深度</span>
              )}
              {DEFAULT_PREREQ_MAP[course.id] && (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400/90 max-w-[120px] truncate" title={`建議先完成：${DEFAULT_PREREQ_MAP[course.id]}`}>
                  先修：{DEFAULT_PREREQ_MAP[course.id]}
                </span>
              )}
            </div>
            <p className="text-white/60 text-sm mb-3 line-clamp-2">{course.description}</p>
            <p className="text-white/40 text-xs mb-2">{course.targetAudience ?? `適合：${DEFAULT_LEVEL_LABELS[course.level]}者`}</p>
...
export default function CourseListVirtualized({ 
  progress, 
  onShowUpgrade,
  courses,
  levelLabels = DEFAULT_LEVEL_LABELS,
  prereqMap = DEFAULT_PREREQ_MAP
}: CourseListVirtualizedProps) {
  ...
  const coursesByLevel = {
    ...
    expert: filteredCourses.filter(c => c.level === 'expert')
  }
  ...
  // Render a virtualized list of courses for a specific level
  const renderVirtualizedLevel = (level: CourseLevel) => {
      ...
      <section key={level} aria-labelledby={`section-${level}`} className="pt-10 md:pt-0 first:pt-0 scroll-mt-20">
        ...
        <div className={viewMode === 'list' ? 'flex flex-col gap-2' : 'grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5 md:gap-6'}>
          <VirtualizedList
            ...
            itemHeight={400} // Approximate height for course cards
            containerHeight={Math.min(1200, visibleCourses.length * 400)} // Show up to 1200px or all items if fewer
  ...
  return (
    <div className="space-y-10 md:space-y-12 mb-12 divide-y divide-white/5 md:divide-y-0 md:space-y-12">
      {renderVirtualizedLevel('beginner')}
      {renderVirtualizedLevel('intermediate')}
      {renderVirtualizedLevel('expert')}
