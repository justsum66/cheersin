...
export function InViewAnimate({
  ...
  const initial = reducedMotion ? { opacity: 1, y: 0, x: 0, scale: 1 } : { opacity: 0, y, x, scale: 0.97 }
  const whileInView = reducedMotion ? { opacity: 1, y: 0, x: 0, scale: 1 } : { opacity: 1, y: 0, x: 0, scale: 1 }
  return (
    <m.div
      initial={initial}
